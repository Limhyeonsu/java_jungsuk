...
public class StreamEx8 {
	public static void main(String[] args) {
		...
		Map<Integer, List<Student4>> stuByBan = Stream.of(stuArr)
				.collect(Collectors.groupingBy(Student4::getBan));	//groupingBy()로 그룹화 하면 기본적으로 List<T>에 담는다
		
		System.out.println(stuByBan);	//{1=[[나자바, 남, 1학년 1반, 300점],...2=[[이지미, 여, 1학년 2반, 150점]..3=[[황지미, 여, 1학년 3반, 100점]..}
		...
		Map<Integer, Map<Integer, List<Student4>>> stuByHakAndBan = 
				Stream.of(stuArr).collect(Collectors.groupingBy(Student4::getHak, Collectors.groupingBy(Student4::getBan)
						));
		
		System.out.println(stuByHakAndBan);  //1={1=[[나자바, 남, 1학년 1반, 300점],.. 2=..3=..}, 2={1=..2=..3=..}, 3={1=..2=..3=..}}
		...
		Map<String, Set<Student4.Level>> stuByScoreGroup = Stream.of(stuArr)
				.collect(Collectors.groupingBy(s->s.getHak() + "-" + s.getBan(), Collectors.mapping(s->{
					if(s.getScore() >= 200) return Student4.Level.HIGH;
					 else if(s.getScore() >= 100) return Student4.Level.MID;
					 else									 return Student4.Level.LOW;
						}, Collectors.toSet())));
